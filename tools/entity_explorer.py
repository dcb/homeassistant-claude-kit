...
def load_device_registry(config_path: Path) -> Dict[str, Optional[str]]:
    """Load device registry and build device_id -> area_id map."""
        ...
        try:
            with open(device_path, "r") as f:
                ...
                for device in device_data.get("data", {}).get("devices", []):
                    device_areas[device["id"]] = device.get("area_id")
        except Exception as e:
...
def load_floor_registry(config_path: Path) -> Dict[str, Dict]:
    """Load floor registry for floor_id -> name/level mapping."""
    floor_path = config_path / ".storage" / "core.floor_registry"
    ...
    if floor_path.exists():
        try:
            with open(floor_path, "r") as f:
                ...
                for floor in floor_data.get("data", {}).get("floors", []):
                    floors[floor["floor_id"]] = {
                        "name": floor.get("name", ""),
                        "level": floor.get("level"),
                    }
        except Exception as e:
            print(f"Warning: Could not load floor registry: {e}")
...
def categorize_entities(
    ...
    device_areas: Optional[Dict[str, Optional[str]]] = None,
) -> Dict:
    """Categorize entities by domain and area.

    Area resolution: entity.area_id (direct) -> device.area_id (via device_id).
    Most entities only have area_id on their parent device, not directly.
    """
    ...
    for entity in entities:
        ...
        # Resolve area: direct entity area_id, then fallback to device area_id
        area_id = entity.get("area_id")
        if not area_id and device_areas and entity.get("device_id"):
            area_id = device_areas.get(entity["device_id"])
...
def main():
    ...
    # Categorize entities (resolves area via device registry when entity has no direct area_id)
