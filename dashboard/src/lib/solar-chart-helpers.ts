...
/** A time-series data point compatible with both useHistory and history-utils. */
...
export function mergeChartData(
  ...
  const solarMap = new Map(solar.filter((p) => p.value != null).map((p) => [p.time, p.value! * toW]));
  const loadMap = new Map(load.filter((p) => p.value != null).map((p) => [p.time, p.value! * toW]));
  const chargerMap = new Map(charger.filter((p) => p.value != null).map((p) => [p.time, p.value! * chargerToW]));
  ...
  const interpolateForecast = (t: number): number | null => {
    ...
    const fv0 = forecast[0].value;
    if (t <= forecast[0].time) return fv0 != null && fv0 > 0 ? fv0 : null;
    const fvLast = forecast[forecast.length - 1].value;
    if (t >= forecast[forecast.length - 1].time) {
      return fvLast != null && fvLast > 0 ? fvLast : null;
    }
    ...
    if (p0.value == null || p1.value == null) return null;
...
export function computeTotals(
  ...
  const integrate = (points: HistoryPoint[], multiplier = 1): number => {
    ...
    for (let i = 1; i < points.length; i++) {
      const v0 = points[i - 1].value;
      const v1 = points[i].value;
      if (v0 == null || v1 == null) continue;
      const dtHours = (points[i].time - points[i - 1].time) / 3_600_000;
      const avgW = ((v1 + v0) / 2) * multiplier;
      wh += avgW * dtHours;
    }
    return wh / 1000;
  };

  // Grid: positive = export, negative = import (Powerwall: site_now positive = exporting)
  let importedKwh = 0;
  let exportedKwh = 0;
  if (grid.length >= 2) {
    for (let i = 1; i < grid.length; i++) {
      const v0 = grid[i - 1].value;
      const v1 = grid[i].value;
      if (v0 == null || v1 == null) continue;
  ...
  if (charger.length >= 2 && solar.length >= 2 && load.length >= 2) {
    const interpolate = (points: HistoryPoint[], t: number, mult: number): number => {
      ...
      if (t <= points[0].time) return v0 != null ? v0 * mult : 0;
      const vLast = points[points.length - 1].value;
      if (t >= points[points.length - 1].time) return vLast != null ? vLast * mult : 0;
      let lo = 0;
      ...
      const p0 = points[lo];
      const p1 = points[hi];
      if (p0.value == null || p1.value == null) return 0;
      const frac = (t - p0.time) / (p1.time - p0.time);
      return (p0.value + frac * (p1.value - p0.value)) * mult;
    };

    for (let i = 1; i < charger.length; i++) {
      const cv0 = charger[i - 1].value;
      const cv1 = charger[i].value;
      if (cv0 == null || cv1 == null) continue;
      const t0 = charger[i - 1].time;
      const t1 = charger[i].time;
      const tMid = (t0 + t1) / 2;
      const dtHours = (t1 - t0) / 3_600_000;

      const chargerW = ((cv0 + cv1) / 2) * chargerToW;
